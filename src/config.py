...
class APIConfig:
	...
	def validate(self) -> None:
		...
		if self.provider not in ["anthropic", "openai"]:
			raise ValueError(f"Invalid API_PROVIDER: {self.provider}. Must be 'anthropic' or 'openai'")
...
class EmbeddingConfig:
	...
	def validate(self) -> None:
		...
		if self.provider not in ["local", "openai"]:
			raise ValueError(f"Invalid EMBEDDING_PROVIDER: {self.provider}. Must be 'local' or 'openai'")
...
class AppConfig:
	...
	def print_summary(self) -> None:
		...
		print(f"RAG Settings: chunk_size={self.rag.chunk_size}, k={self.rag.retrieval_k}")
		print("="*80 + "\n")
